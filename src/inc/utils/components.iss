[Code]
type
  TSelectedComponentsMode = (scmAll, scmExcludeDisabledItems);
...
function GetSelectedComponentsCount(const Mode: TSelectedComponentsMode): Integer;
var
  S: TStringList;
  i: Integer;
...
begin
  ...
  case Mode of
    ...
    scmExcludeDisabledItems:
      begin
        for i := 0 to GetComponentsListCount - 1 do
        begin
          if WizardForm.ComponentsList.ItemEnabled[i] and WizardForm.ComponentsList.Checked[i] then
            Inc(Result);
        end; 
      end;
...
function GetComponentsListItemIndex(const Text: String;
